...
class PID(nn.Module):
    ...
    def forward(self, setpoint, system_output):
        ...
        if time.time() - self.last_update > 1:
            ...
            if self.adaptive:
                self.adapt()
            logger.info(f"Setpoint: {setpoint}, System Output: {system_output}, Error: {self.error}")
            ...
            self.input_history = torch.cat((self.input_history, torch.tensor([system_output])))
            self.set_point_history = torch.cat((self.set_point_history, torch.tensor([setpoint])))
            self.output_history = torch.cat((self.output_history, torch.tensor([output])))
            self.error_history = torch.cat((self.error_history, torch.tensor([self.error])))
        ...
        if len(self.error_history) > 5:
            if all([abs(e) > self.threshold for e in self.error_history[-5:]]) and not self.adaptive:
                self.adaptive = True
                logger.info("Adaptive learning enabled")
            if all([abs(e) <= self.threshold for e in self.error_history[-5:]]) and self.adaptive:
                self.adaptive = False
    ...
    def evaluate(self, input_dict):
        return {"new_input": self.forward(input_dict["setpoint"], input_dict["system_output"])}
